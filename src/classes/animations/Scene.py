...
class Rotation:
    ...
    def set_white(self, white:int):
        ...
        self.white_string = str(format_to_hex(white)) + str(format_to_hex(white))
        return self
...
if __name__ == "__main__":
    print(str(format_to_hex(1000)))
    # an interactive way to create scenes
    scene = Scene(input("Enter the name of the scene : "))
    while True:
        rotation = Rotation()
        white_bool = "j"
        while white_bool not in ["y", "n"] :
            white_bool = input("Is the color white ? (Y/N) : ").lower()
            if white_bool not in ["y", "n"]:
                print("Invalid input. Please enter Y or N.")
        if white_bool == "y":
            while True:
                try:
                    white = int(input("White (0-1000): "))
                    if 0 <= white <= 1000:
                        rotation.set_white(white)
                        break
                    else:
                        print("Invalid white value. Please enter a value between 0 and 1000.")
                except ValueError:
            ...
            while True:
                try:
                    ...
                    if 0 <= hue <= 360 and 0 <= saturation <= 1000 and 0 <= brightness <= 1000:
                        rotation.set_color(hue, saturation, brightness)
                        break
                    else:
