...
SNAPSHOT_PATH = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(os.path.dirname(__file__))))) + "/snapshot.json"
SCENES_PATH = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(os.path.dirname(__file__))))) + "/scenes.json"
...
def get_all_devices() -> dict:
    ...
    for device in data:
        try:
            if device["ip"] == "":
                print("No ip address for device")
                continue
            if device["err"]:
                print("Error in device")
                continue
            if "1" not in device["dps"]["dps"]:
                device_instantiation = tinytuya.BulbDevice(
            ...
            if "1" in device["dps"]['dps']:
                device_instantiation = tinytuya.OutletDevice(
                    device["id"], device["ip"], device["key"])
                device_instantiation.set_version(3.3)
...
def main():
    ...
    while True:
        ...
        if status['dps']['2'] != current_brighness_status:
            ...
            change_scene(simple_brightness(
                status['dps']['2']), scenes, devices["bulbs"])
